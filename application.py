...
# =====================
    ...
    # --- Saisie date de départ pour le solde ---
    date_depart = st.date_input(
        "📅 Date de départ pour le solde de trésorerie",
        value=pd.to_datetime("2025-04-01")
    )

    # Filtrer les mouvements bancaires jusqu'à cette date
    comptes_bancaires = df_pivot[
        (df_pivot["Compte"].astype(str).str.startswith("5")) &
        (pd.to_datetime(df_pivot["Date"], errors="coerce") <= pd.to_datetime(date_depart))
    ]

    # Calcul du solde de départ
    solde_depart = comptes_bancaires["Crédit"].sum() - comptes_bancaires["Débit"].sum()
    st.info(f"💰 Solde de départ calculé au {date_depart}: {solde_depart:,.2f} €")
    ...
    if st.button("📊 Générer la prévision de trésorerie"):
        try:
            ...
            # Ajout prévisions pour les mois futurs
            ...
            # Calcul du solde cumulé à partir du solde de départ
